...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Modifier le coupon: {{ $coupon->code }}</h1>
            @if($coupon->times_used > 0)
                <span class="px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">
                    Utilisé {{ $coupon->times_used }} fois
                </span>
            @endif
        </div>
        ...
        <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $coupon->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div id="value-field">
                    ...
                    <input type="number" name="value" id="value" value="{{ old('value', $coupon->value) }}" step="0.01" min="0"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                           required>
                    <p class="mt-1 text-xs text-gray-500" id="value-help"></p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="datetime-local" name="valid_from" id="valid_from" value="{{ old('valid_from', $coupon->valid_from->format('Y-m-d\TH:i')) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                           required>
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="valid_until" id="valid_until" value="{{ old('valid_until', $coupon->valid_until->format('Y-m-d\TH:i')) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                           required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="usage_limit" id="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}"
                           ...
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @if($coupon->times_used > 0)
                        <p class="mt-1 text-xs text-blue-600">Déjà utilisé {{ $coupon->times_used }} fois</p>
                    @endif
                </div>
            ...
            <!-- Categories -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Restreindre à des catégories spécifiques</label>
                                ...
                                @foreach($category->children as $child)
                                    <label class="flex items-center mb-2">
                                        <input type="checkbox" name="category_ids[]" value="{{ $child->id }}"
                                               {{ in_array($child->id, old('category_ids', $coupon->category_ids ?? [])) ? 'checked' : '' }}
                                               class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
